...
asked to complete is called the *prompt*. When the model is working, some
...
prompt them. This is *prompt engineering*. Often, the bot provider
initializes the bot with a hidden or secret prompt before the user
...
2. They do not truly reason, they make smart guesses based
   on data they have seen. For example: there's clearly a ton of python
   code in gpt-4. When asked to solve a programming problem in python it
   often does well. However, when asked to solve the same problems in
   common lisp, it is pretty likely to give wrong answers. I believe
   this is because Lisp is different from python, and 
   there's a lot less training data (sample code) written in lisp. But
   if gpt were truly reasoning, if it knew how to solve the problem in
   python, it would be able to write correct lisp code.
...
part of computing in my adult lifetime. So models will get better and
better, fast. I suspect that by Fall 2023 we'll be looking back on GPT-4
as quaint compared to state-of-the-art.

Training will continue to be expensive and require supercomputing-level
resoruces --- it costs many millions to train these models, because they
consume huge amounts of data, and the training is very computationally
intense. In fact training costs will rise as training datasets expand and
model architectures grow.
...
trained will get cheaper and cheaper while the cost of training
continues to rise.^[There's an analog in video streaming: codecs are
frequently designed so that compression is very CPU intensive but
decoding/playing is not.] There are a growing number of
...
openness practices to end.

There's no evidence that factuality problems will go away. Improving the
*quality* of training data will help. Having the resources to curate
training data sets will distinguish the most successful model purveyors.
...
Examples
    ...
    -   What bing is doing (note that adding footnotes is a way to
        counter factuality/correctness concerns)
    -   chatgpt
